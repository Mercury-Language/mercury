...
% both the semidet predicate date_from_string/2, and its det function
% version, date_from_string/1.
%
...
test_valid_date_time(CheckRoundTrip, Test, !IO) :-
    ...
    ( if date_from_string(TestStr, DateTimeP) then
        MaybeDateTimeP = yes(DateTimeP),
        RoundTripStr = date_to_string(DateTimeP),
        (
            CheckRoundTrip = yes,
    ...
    ( try []
        DateTimeF = det_date_from_string(TestStr)
    then
...
test_invalid_date_time(Test, !IO) :-
    ...
    ( if date_from_string(TestStr, DateTimeP) then
        MaybeDateTimeP = yes(DateTimeP),
        DateTimePStr = date_to_string(DateTimeP),
