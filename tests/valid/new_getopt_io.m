...
:- type short_option(OptionType) ==
    (pred(char::in, OptionType::out) is semidet).

:- type long_option(OptionType) ==
    (pred(string::in, OptionType::out) is semidet).

:- type option_default_value_nondet(OptionType) ==
    (pred(OptionType::out, option_data::out) is nondet).
:- type option_default_value_multi(OptionType) ==
    (pred(OptionType::out, option_data::out) is multi).

:- type special_handler(OptionType) ==
    (pred(OptionType::in, special_data::in,
...
:- type option_ops(OptionType)
    --->    option_ops(
                ...
                option_default_value_nondet(OptionType)
            )
    ;       option_ops(
                short_option(OptionType),
                long_option(OptionType),
    ...
    ;       option_ops_multi(
                short_option(OptionType),
                long_option(OptionType),
                option_default_value_multi(OptionType)
            )
    ;       option_ops_multi(
                short_option(OptionType),
                long_option(OptionType),
...
:- type user_data_handler(OptionType, UserDataType) ==
    (pred(OptionType::in, special_data::in,
        option_table(OptionType)::in, maybe_option_table(OptionType)::out,
        UserDataType::in, UserDataType::out) is semidet).
...
:- pred process_options_io(option_ops(OptionType)::in,
    ...
    %
:- pred process_options_track(option_ops_track(OptionType)::in,
    list(string)::in, list(string)::out, list(string)::out,
    option_table(OptionType)::in, maybe_option_table_se(OptionType)::out,
    set(OptionType)::out) is det.
:- pred process_options_track_io(option_ops_track(OptionType)::in,
    list(string)::in, list(string)::out, list(string)::out,
    option_table(OptionType)::in, maybe_option_table_se(OptionType)::out,
...
:- pred process_options_userdata(
    option_ops_userdata(OptionType, UserDataType)::in,
    list(string)::in, list(string)::out, list(string)::out,
...
%---------------------------------------------------------------------------%
    ...
    %
:- pred init_option_table(option_default_value_nondet(OptionType)::in,
    option_table(OptionType)::out) is det.
:- pred init_option_table_multi(option_default_value_multi(OptionType)::in,
    option_table(OptionType)::out) is det.
...
:- type maybe_option_error(OptionType)
    ...
    %
:- pred record_arguments(short_option(OptionType)::in,
    long_option(OptionType)::in, option_table(OptionType)::in,
    list(string)::in, list(string)::out, list(string)::out,
...
%---------------------------------------------------------------------------%
    ...
    % *except* ov_file_special. (The _nfs suffix is for "non-file-special.)
:- type option_value_nfs(OptionType) =< option_value(OptionType)
    --->    ov_bool(OptionType, string, bool)
    ;       ov_int(OptionType, string, int)
    ;       ov_string(OptionType, string, string)
    ;       ov_maybe_int(OptionType, string, maybe(int))
    ...
    % option values.
    %
:- pred expand_file_specials(short_option(OptionType)::in,
    long_option(OptionType)::in, option_table(OptionType)::in,
    list(option_value(OptionType))::in, maybe_option_error(OptionType)::out,
    list(option_value_nfs(OptionType))::out, io::di, io::uo) is det.
...
:- type option_ops_special(OptionType, UserDataType)
    ...
    ;       notrack(
                pred(OptionType::in, special_data::in,
                    ...
                    maybe_option_table(OptionType)::out) is semidet
            )
    ;       track(
                pred(OptionType::in, special_data::in,
                    option_table(OptionType)::in,
...
:- pred option_ops_to_internal_and_option_table(option_ops(OptionType)::in,
    short_option(OptionType)::out, long_option(OptionType)::out,
    option_ops_special(OptionType, UserDataType)::out,
    option_table(OptionType)::out) is det.
...
:- pred process_arguments(
    short_option(OptionType)::in,
    long_option(OptionType)::in,
    option_ops_special(OptionType, UserDataType)::in,
    list(string)::in, list(string)::out, list(string)::out,
    maybe_option_error(OptionType)::out,
    option_table(OptionType)::in, option_table(OptionType)::out,
...
process_arguments(ShortOptionPred, LongOptionPred, SpecialHandler,
    ...
    %
    % - In phase 1, record_arguments processes all the arguments, classifies
    %   them into option arguments and non-option arguments, and then
    %   recognizes and records the options (and if relevant, their values)
    %   in option arguments.
    %
    ...
    %   (if relevant) as we go, and filling in !OptionsSet.
    %
...
:- pred record_arguments_loop(short_option(OptionType)::in,
    long_option(OptionType)::in, option_table(OptionType)::in,
    list(string)::in, list(string)::out,
    list(string)::in, list(string)::out,
    maybe_option_error(OptionType)::out,
...
:- pred record_unnegated_short_options(
    short_option(OptionType)::in, option_table(OptionType)::in,
...
:- pred expand_file_specials_check(short_option(OptionType)::in,
    long_option(OptionType)::in, option_table(OptionType)::in,
    list(option_value(OptionType))::in, set(string)::in,
    maybe_option_error(OptionType)::out,
    list(option_value_nfs(OptionType))::out, MaybeIO::di, MaybeIO::uo) is det
    <= read_file_contents(MaybeIO).

expand_file_specials_check(ShortOptionPred, LongOptionPred, OptionTable,
        OVs, SeenFileNames, MaybeError, NFSOVs, !MaybeIO) :-
    expand_file_specials_loop(ShortOptionPred, LongOptionPred, OptionTable,
        OVs, SeenFileNames, MaybeError, cord.init, NFSOVsCord, !MaybeIO),
    NFSOVs = cord.list(NFSOVsCord).

:- pred expand_file_specials_loop(short_option(OptionType)::in,
    long_option(OptionType)::in,
    option_table(OptionType)::in, list(option_value(OptionType))::in,
    set(string)::in, maybe_option_error(OptionType)::out,
    cord(option_value_nfs(OptionType))::in,
    cord(option_value_nfs(OptionType))::out,
...
expand_file_specials_loop(ShortOptionPred, LongOptionPred, OptionTable,
    ...
    (
        ...
        cord.snoc(coerce(OV0), !NFSOVsCord),
        expand_file_specials_loop(ShortOptionPred, LongOptionPred, OptionTable,
    ...
    ;
        ...
        ( if set.insert_new(FileName, SeenFileNames0, SeenFileNames) then
            ...
            (
                ...
                !:NFSOVsCord = !.NFSOVsCord ++ NFSFileOVsCord,
...
:- type file_options_or_error(OptionType)
    --->    file_options_no_error(cord(option_value_nfs(OptionType)))
    ;       file_options_error(option_error(OptionType)).

:- pred expand_file_special_option(short_option(OptionType)::in,
    long_option(OptionType)::in, option_table(OptionType)::in,
    set(string)::in, OptionType::in, string::in, string::in,
    file_options_or_error(OptionType)::out, MaybeIO::di, MaybeIO::uo) is det
    <= read_file_contents(MaybeIO).
...
:- pred process_option_values(option_ops_special(OptionType, UserDataType)::in,
    list(option_value_nfs(OptionType))::in,
    maybe_option_error(OptionType)::out,
    option_table(OptionType)::in, option_table(OptionType)::out,
    set(OptionType)::in, set(OptionType)::out,
    UserDataType::in, UserDataType::out) is det.
