...
render(Params1) -->
    ...
    ( { Res = ok } ->
        ...
        % Render the image
        { render_rows(Params2, 0, Wid, Ht, cord.empty, Bytes) },

        % Write out the image.
        foldl_pred((pred(Byte::in, IO0::di, IO::uo) is det :-
...
:- pred render_rows(render_params::in, int::in, int::in, int::in, 
    cord(int)::in, cord(int)::out) is det.

render_rows(Params, StartRow, Width, Height, !Bytes) :-
    ( StartRow = Height ->
        true
    ;
        (
            render_row(Params, 0, StartRow, Width, cord.empty, RowBytes)
        & 
            render_rows(Params, StartRow+1, Width, Height, cord.empty, RowsBytes)
        ),
        % This isn't tail recursive,  We can make it tail recursive but that
        % would make the above conjunction dependant.
        !:Bytes = !.Bytes ++ RowBytes ++ RowsBytes
    ).

:- pred render_row(render_params::in, int::in, int::in, int::in,
    cord(int)::in, cord(int)::out) is det.

render_row(Params, StartCol, Row, Width, !Bytes) :-
    ( StartCol = Width ->
        true
    ;
        Pixel = pixel_coords(StartCol, Row),
        ...
        !:Bytes = !.Bytes ++ from_list([R, G, B]),

        render_row(Params, StartCol + 1, Row, Width, !Bytes)
    ).
