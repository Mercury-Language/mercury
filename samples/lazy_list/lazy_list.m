...
%-----------------------------------------------------------------------------%
    ...
    % append(Start, End) returns the result of concatenating Start and End.
    %
...
append([X | Xs], Ys) = [X | delay((func) = append(force_list(Xs), Ys))].
...
iterate(F, X0) = [X0 | delay((func) = iterate(F, F(X0)))].

take(_, []) = [].
take(N, [X | Xs]) =
    (if N > 0 then
        [X | delay((func) = take(N - 1, force_list(Xs)))]
    else
        []
    ).
