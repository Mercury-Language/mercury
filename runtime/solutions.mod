...
*/
	...
	mkframe("builtin_solutions", 1,
		LABEL(mercury__builtin_solutions_2_0_i2));
	...
	call_nondet_closure(LABEL(mercury__builtin_solutions_2_0_i1),
			LABEL(mercury__builtin_solutions_2_0));

mercury__builtin_solutions_2_0_i1:
	/* we found a solution */
	/* insert it into the list, and then look for the next one */
	framevar(0) = list_cons(r1, framevar(0));
	redo();

mercury__builtin_solutions_2_0_i2:
	/* no more solutions */
