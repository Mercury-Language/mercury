...
** pointer to a cell allocated at time t' > t).  This assumption is
** valid for many declarative programming languages.
** 
** Cells are allocated in 4 Kbyte "pages" (allocated on 4 KByte
** boundaries).  Cells are powers of two in size and are allocated on the
** corresponding address boundaries.  Each page contains cells of the same
** size "payload" (i.e., the user accessible portion of the cell).  The
** first cell in each page is reserved for metadata, specifically the
** payload size and bit mask used to convert a pointer into the page to a
** pointer to the corresponding cell in the page.
** 
...
** for a mark stack.
** 
** Special handling is required for cells that contain mutable data (i.e.,
** which may contain pointers to more recently allocated cells) and for
** "big" cells (i.e., those for which the big-bag-o-pages approach doesn't
** scale).
** 
** NOTES
**
...
** only non-pointer data (and thereby avoid marking through them).  This
** could be useful for allocating strings and arrays of numbers.
*/
...
static hgc_int growth_threshold = 20;   /* Grow small heap if more than
                                        ...
                                        */
static hgc_int growth_factor = 50;      /* Percent growth for
                                        ** small_high_water
                                        */
...
find_cell(hgc_ptr p, hgc_ptr *cl_ref, hgc_ptr *payload_ref,
        hgc_int *payload_size_ref)
{
    return  find_small_cell(p, cl_ref, payload_ref, payload_size_ref) ||
...
mercury_hgc_init(void)
{
    ...
    /*
    ** Just return if we've already been initialised.
    */
    if (hgc_initialised) {
        return;
    }
...
mercury_hgc_free(void *p)
{
    ...
    if (find_small_cell(p, &cl, &payload, &payload_size)) {
        hgc_ptr payload_ptr;
        for (payload_ptr = payload; payload_ptr < payload + payload_size; payload_ptr++) {
            *payload_ptr = 0;
        }
        return;
    }
