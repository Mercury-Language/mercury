...
:- pred expand_eqv_types_insts(
    ...
    type_eqv_map::out, used_eqv_modules::out,
...
%---------------------------------------------------------------------------%
    ...
    % is circular.
    %
    % Defined and exported for decide_type_repn.m. It is not used by
    % any code in this module, but "culturally" it belongs here.
    %
...
expand_eqv_types_insts(AugCompUnit0, AugCompUnit, EventSpecMap0, EventSpecMap,
    ...
    !:UsedModules = used_eqv_modules_init,
    !:Specs = [],
...
:- pred replace_in_parse_tree_module_src(equiv_params::in,
    ...
    maybe(recompilation_info)::in, maybe(recompilation_info)::out,
    used_eqv_modules::in, used_eqv_modules::out,
    list(diag_spec)::in, list(diag_spec)::out) is det.
...
:- pred replace_in_type_defn_info_general(
    pred(equiv_params, maybe_record_sym_name_use, type_ctor, prog_context,
        ...
        used_eqv_modules, used_eqv_modules, list(diag_spec))
    ...
    maybe(recompilation_info)::in, maybe(recompilation_info)::out,
    used_eqv_modules::in, used_eqv_modules::out, list(diag_spec)::out) is det.
...
replace_in_type_repn_info(Params, MaybeRecord, TypeRepnInfo0, TypeRepnInfo,
    ...
    ;
        ...
        % Construct a type from the type ctor, substituting 'void'
        % for any type parameters, so that we can call
        % replace_in_type_maybe_record_use.
        % We do not care about the type arguments,
        % so we can drop them again afterwards.
...
replace_in_type_repn_eqv(TypeEqvMap, TypeRepnInfo0, TypeRepnInfo, !Specs) :-
    ...
    replace_in_type_maybe_record_use(TypeEqvMap, do_not_record_sym_name_use,
        ...
        no_item_recomp_deps, _, used_eqv_modules_init, _),
    set.to_sorted_list(Circ, CircTypes),
    (
        CircTypes = [_ | _],
        !:Specs = [report_circular_eqv_type(TypeCtor, Context) | !.Specs]
...
replace_in_class_interface(Params, MaybeRecord,
        ...
        !Specs) :-
    list.map_foldl3(replace_in_class_decl(Params, MaybeRecord),
...
%
%       replace_in_list(Params, MaybeRecord, replace_in_instance_method,
...
%---------------------------------------------------------------------------%
%
% The rest of this module consists of utility predicates
% that are useful in handling of more than one kind item.
%
%---------------------------------------------------------------------------%
%---------------------------------------------------------------------------%

    % This predicate is used when handling both
...
replace_in_with_inst(Params, MaybeRecord, PredSymName, PredFormArity, Context,
    ...
    (
        ...
        InstEqvMap = Params ^ ep_inst_eqv_map,
        replace_in_inst(InstEqvMap, MaybeRecord, WithInst0, WithInst,
            !ItemRecompDeps, !UsedModules),
        ( if
...
:- func report_circular_eqv_type(type_ctor, prog_context) = diag_spec.
...
report_circular_eqv_type(TypeCtor, Context) = Spec :-
    Pieces = [words("Error: equivalence type")] ++
        ...
        [words("is")] ++ color_as_incorrect([words("circular.")]) ++ [nl],
