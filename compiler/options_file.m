...
% in this module that generates an err_spec with severity_warning,
...
:- pred pathname_occurs_in_incl_stack(incl_stack::in, file_name::in,
    term_context::in, err_spec::out) is semidet.
...
:- func io_error_to_parse_error(file_name, int, io.error) = err_spec.
...
:- func report_split_error(file_name, int, string) = err_spec.
    ...
    (
        ...
        ( if map.search(EnvMap, VarName, EnvStr) then
            ...
            ;
                ...
                Spec = split_error_msg_to_err_spec(VarName, Msg),
                !:ParseSpecs = [Spec | !.ParseSpecs]
            )
        else
...
:- pred report_any_undefined_variables(file_name::in, int::in,
    set(string)::in, list(warn_spec)::in, list(warn_spec)::out) is det.
...
lookup_mmc_maybe_module_options(Variables, EnvOptFileVarClass, Result) :-
    ...
    ;
        ...
        % Returning error1 here is correct because all err_specs in Specs
        % will have severity_error. There is (as of 2022 jan 23) exactly
        % one place in this module that generates an err_spec whose
...
:- type variable_result(T)
    --->    var_result_set(T)
    ;       var_result_unset
    ;       var_result_error(one_or_more(err_spec)).
...
:- func split_error_msg_to_err_spec(string, string) = err_spec.

split_error_msg_to_err_spec(VarName, Msg) = Spec :-
