...
generate_code(Lang, MLDS, !IO) :-
	...
	generate_namespace_details(Lang, ClassName, NameSpaceFmtStr,
		Namespace),
...
:- pred write_statement(foreign_language::in(managed_lang),
	mlds__arguments::in, mlds__statement::in, io::di, io::uo) is det.
...
:- pred write_rval(foreign_language::in(managed_lang), mlds__rval::in,
	io::di, io::uo) is det.
...
:- pred write_rval_const(foreign_language::in(managed_lang),
	mlds__rval_const::in, io::di, io::uo) is det.
...
:- pred write_lval(foreign_language::in(managed_lang), mlds__lval::in,
	io::di, io::uo) is det.
...
write_input_arg_as_foreign_type(Lang, Arg, !IO) :-
	...
	write_il_type_as_foreign_type(Lang,
		mlds_type_to_ilds_type(DataRep, Type), !IO),
	io__write_string(" ", !IO),
