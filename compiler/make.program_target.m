    ...
    ;
        ...
        (
            ...
            (
                ...
                % We can only build the .init file if we have successfully
                % built the .c files.
                (
                    SharedLibsSucceeded = yes,
                    % Errors while making the .init file should be very rare.
                    io.output_stream(ErrorStream, !IO),
                    make_init_file(ErrorStream, MainModuleName, AllModules,
                        Succeeded, !IO)
                ;
...
maybe_install_library_file(Linkage, FileName, InstallDir, Succeeded, !IO) :-
    ...
    ( list.member(Linkage, LibLinkages) ->
        ...
        (
            ...
            Succeeded0 = yes
        ->
            % FileName is directory qualified so strip that qualification
            % off.
            %
