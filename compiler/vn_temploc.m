...
:- pred vn_temploc__init_templocs(int, int, vnlvalset, vn_tables, templocs).
:- mode vn_temploc__init_templocs(in, in, in, in, out) is det.

	% Get a temporary location.

:- pred vn_temploc__next_temploc(templocs, templocs, vnlval).
% :- mode vn_temploc__next_temploc(di, uo, out) is det.
:- mode vn_temploc__next_temploc(in, out, out) is det.

	% Prevent the use of this location as a temporary.

:- pred vn_temploc__no_temploc(vnlval, templocs, templocs).
% :- mode vn_temploc__no_temploc(in, di, uo) is det.
...
% :- mode vn_temploc__reuse_templocs(in, di, uo) is det.
:- mode vn_temploc__reuse_templocs(in, in, out) is det.

	% Give the number of the highest temp variable used.

:- pred vn_temploc__max_temploc(templocs, int).
:- mode vn_temploc__max_temploc(in, out) is det.
...
vn_temploc__init_templocs(MaxTemp, MaxReg, Livevals, VnTables, Templocs) :-
	vn_temploc__get_n_temps(1, MaxTemp, Temps),
	vn_temploc__find_free_regs(1, MaxReg, Livevals, VnTables, Regs),
	list__append(Regs, Temps, Queue),
	NextTemp is MaxTemp + 1,
	Templocs = quad(Queue, Livevals, 0, NextTemp).

vn_temploc__next_temploc(quad(Queue0, Livevnlvals, MaxUsed0, Next0),
...
vn_temploc__no_temploc(Vnlval, quad(Queue0, Livevnlvals, MaxUsed, Next),
...
vn_temploc__max_temploc(quad(_Queue0, _Livevnlvals, MaxUsed, _Next), MaxUsed).

	% Return the non-live registers from the first N registers.

:- pred vn_temploc__find_free_regs(int, int, vnlvalset, vn_tables, list(vnlval)).
:- mode vn_temploc__find_free_regs(in, in, in, in, out) is det.

vn_temploc__find_free_regs(N, Max, Livevals, VnTables, Freeregs) :-
	( N > Max ->
		Freeregs = []
	;
		N1 is N + 1,
...
:- pred vn_temploc__get_n_temps(int, int, list(vnlval)).
:- mode vn_temploc__get_n_temps(in, in, out) is det.

vn_temploc__get_n_temps(N, Max, Temps) :-
	( N > Max ->
		Temps = []
	;
		N1 is N + 1,
		vn_temploc__get_n_temps(N1, Max, Temps0),
