...
%---------------------------------------------------------------------------%
%
% File: term_osi_main.m.
% Main author: crs (when this file was named termination.m).
...
:- import_module termination.term_osi.term_osi_pass1.
...
special_pred_id_to_termination(SpecialPredId, HeadVars, ArgSize,
    ...
    (
        ...
        term_osi_util.make_bool_list(HeadVars, [no, no, no], OutList),
        ArgSize = finite(0, OutList),
        Termination = cannot_loop(unit)
    ;
        SpecialPredId = spec_pred_unify,
        term_osi_util.make_bool_list(HeadVars, [yes, yes], OutList),
        ArgSize = finite(0, OutList),
        Termination = cannot_loop(unit)
    ;
        SpecialPredId = spec_pred_index,
        term_osi_util.make_bool_list(HeadVars, [no, no], OutList),
        ArgSize = finite(0, OutList),
        Termination = cannot_loop(unit)
    ).
...
set_builtin_terminates([ProcId | ProcIds], PredId, PredInfo, ModuleInfo,
    ...
    ( if all_args_input_or_zero_size(ModuleInfo, PredInfo, ProcInfo0) then
        ...
        proc_info_get_headvars(ProcInfo0, HeadVars),
        term_osi_util.make_bool_list(HeadVars, [], UsedArgs),
