...
code_info__get_next_label(Label, !CI) :-
    ...
    code_util__make_internal_label(ModuleInfo, PredId, ProcId, N, Label).
...
code_info__generate_branch_end(StoreMap, MaybeEnd0, MaybeEnd, Code, !CI) :-
    ...
    ;
        ...
        code_info__set_temps_in_use(TempsInUse, EndCodeInfoA, EndCodeInfo)
    ),
...
code_info__after_all_branches(StoreMap, MaybeEnd, !CI) :-
    (
        ...
        code_info__reset_to_position(position_info(BranchEndCodeInfo), !CI),
        code_info__remake_with_store_map(StoreMap, !CI)
    ;
        MaybeEnd = no,
...
:- pred code_info__generate_resume_point(resume_point_info::in,
    ...
    % resume point.
    %
:- pred code_info__resume_point_vars(resume_point_info::in,
...
code_info__prepare_for_disj_hijack(CodeModel, HijackInfo, Code, !CI) :-
    ...
    ;
        ...
        code_info__acquire_temp_slot(lval(redoip(lval(maxfr))), RedoipSlot,
            !CI),
        code_info__acquire_temp_slot(lval(redofr(lval(maxfr))), RedofrSlot,
            !CI),
...
code_info__generate_semi_commit(SemiCommitInfo, Code, !CI) :-
    ...
    ;
        ...
        code_info__pick_stack_resume_point(TopResumePoint, _, StackLabel),
        StackLabelConst = const(code_addr_const(StackLabel)),
    ...
    code_info__maybe_restore_trail_info(MaybeTrailSlots, CommitTrailCode,
        RestoreTrailCode, !CI),
...
code_info__generate_failure(Code, !CI) :-
    ...
    (
        ...
        stack__top_det(ResumePoints, TopResumePoint),
        (
            code_info__pick_matching_resume_addr(!.CI, TopResumePoint,
                FailureAddress0)
        ->
            FailureAddress = FailureAddress0,
            PlaceCode = empty
        ;
            code_info__pick_first_resume_point(TopResumePoint, Map,
                FailureAddress),
            map__to_assoc_list(Map, AssocList),
            code_info__remember_position(!.CI, CurPos),
            code_info__pick_and_place_vars(AssocList, _, PlaceCode, !CI),
...
code_info__maybe_restore_trail_info(MaybeTrailSlots, CommitCode, RestoreCode,
        !CI) :-
    (
        MaybeTrailSlots = no,
        CommitCode = empty,
...
code_info__make_vars_forward_live_2([Var | Vars], StackSlots, N0,
    ...
    code_info__make_vars_forward_live_2(Vars, StackSlots, N1, !VarLocnInfo).
...
code_info__acquire_reg(Type, Lval, !CI) :-
    code_info__get_var_locn_info(!.CI, VarLocnInfo0),
    require(unify(Type, r), "code_info__acquire_reg: unknown reg type"),
...
code_info__setup_call(GoalInfo, ArgInfos, LiveLocs, Code, !CI) :-
    ...
    ;
        ...
        ( CodeModel = model_non ->
                ...
                % the resume point.
            code_info__compute_resume_var_stack_locs(!.CI, ResumeVarLocs),
...
code_info__generate_call_stack_vn_livevals(CI, OutputArgs, LiveVals) :-
    ...
    code_info__generate_stack_var_vn(CI, LiveVarList, set__init, LiveVals1),
    code_info__get_active_temps_data(CI, Temps),
...
code_info__get_variable_slot(CI, Var, Slot) :-
    ...
    ;
        ...
        string__append_list(["code_info__get_variable_slot: variable `",
