...
:- pred mlds_to_java__output_mlds(module_info::in, mlds::in, io::di, io::uo)
    is det.
...
mangle_mlds_sym_name_for_java(qualified(ModuleName0, PlainName), QualKind,
    ...
    string__append_list([MangledModuleName, QualifierOp, JavaSafePlainName],
        JavaSafeName).
...
generate_call_method(CodeAddr, MethodDefn) :-
    ...
    ;
        CodeAddr = mlds__internal(ProcLabel, _SeqNum, OrigFuncSignature)
    ),
    ...
    ReturnEntityDefn = mlds__data(ReturnVarType, no_initializer, GCTraceCode),
...
pred_label_string(pred(PredOrFunc, MaybeDefiningModule, Name, PredArity,
    ...
    PredLabelStr0 = MangledName ++ "_" ++ string__int_to_string(OrigArity)
        ++ "_" ++ Suffix,
    (
...
:- pred output_defns(indent::in, module_info::in, mlds_module_name::in,
    ctor_data::in, mlds__defns::in, io::di, io::uo) is det.
...
:- pred output_defn(indent::in, module_info::in, mlds_module_name::in,
    ctor_data::in, mlds__defn::in, io::di, io::uo) is det.
...
:- pred output_defn_body(indent::in, module_info::in,
    mlds__qualified_entity_name::in, ctor_data::in, mlds__context::in,
    mlds__entity_defn::in, io::di, io::uo) is det.

output_defn_body(_, ModuleInfo, Name, _, _, mlds__data(Type, Initializer, _),
        !IO) :-
...
output_defn_body(Indent, ModuleInfo, Name, CtorData, Context,
    ...
    output_func(Indent, ModuleInfo, Name, CtorData, Context, Signature,
        MaybeBody, !IO).
output_defn_body(Indent, ModuleInfo, Name, _, Context, mlds__class(ClassDefn),
        !IO) :-
...
:- pred output_class(indent::in, module_info::in,
    mlds__qualified_entity_name::in, mlds__context::in, mlds__class_defn::in,
    io::di, io::uo) is det.
...
output_interface(Interface, !IO) :-
    (
        Interface = class_type(qual(ModuleQualifier, QualKind, Name), Arity, _)
    ->
...
:- pred output_class_body(indent::in, module_info::in, mlds__class_kind::in,
    mlds__qualified_entity_name::in, mlds__defns::in,
    mlds_module_name::in, io::di, io::uo) is det.

output_class_body(Indent, ModuleInfo, mlds__class, _, AllMembers, ModuleName,
        !IO) :-
    CtorData = none,    % Not a constructor.
    output_defns(Indent, ModuleInfo, ModuleName, CtorData, AllMembers, !IO).

output_class_body(_Indent, _, mlds__package, _Name, _AllMembers, _, _, _) :-
    unexpected(this_file, "cannot use package as a type.").

output_class_body(Indent, ModuleInfo, mlds__interface, _, AllMembers,
        ModuleName, !IO) :-
    CtorData = none,  % Not a constructor.
    output_defns(Indent, ModuleInfo, ModuleName, CtorData, AllMembers, !IO).

output_class_body(_Indent, _, mlds__struct, _, _AllMembers, _, _, _) :-
    unexpected(this_file, "output_class_body: structs not supported in Java.").

output_class_body(Indent, ModuleInfo, mlds__enum, Name, AllMembers, _, !IO) :-
...
:- pred output_enum_constants(indent::in, module_info::in,
    mlds_module_name::in, mlds__defns::in, io::di, io::uo) is det.
...
:- pred output_func(indent::in, module_info::in, qualified_entity_name::in,
    ctor_data::in, mlds__context::in, func_params::in, function_body::in,
    io::di, io::uo) is det.
...
output_type(mlds__native_int_type, !IO) :-
    io__write_string("int", !IO).
output_type(mlds__native_float_type, !IO) :-
    io__write_string("double", !IO).
output_type(mlds__native_bool_type, !IO) :-
    io__write_string("boolean", !IO).
output_type(mlds__native_char_type, !IO)  :-
...
output_statements(Indent, ModuleInfo, FuncInfo, [Statement | Statements],
    ...
    ( set__member(can_fall_through, StmtExitMethods) ->
        output_statements(Indent, ModuleInfo, FuncInfo, Statements,
            StmtsExitMethods, !IO),
        ExitMethods0 = StmtExitMethods `set__union` StmtsExitMethods,
        ...
        ;
            ...
            ExitMethods = ExitMethods0 `set__delete` can_fall_through
        )
    ;
...
:- pred output_stmt(indent::in, module_info::in, func_info::in, mlds__stmt::in,
...
output_unboxed_result(ModuleInfo, Type, ResultIndex, !IO) :-
    ( java_builtin_type(ModuleInfo, Type, _, JavaBoxedName, UnboxMethod) ->
        ...
        io__write_string(JavaBoxedName, !IO),
        io__write_string(") ", !IO),
        io__format("result[%d]).%s()", [i(ResultIndex), s(UnboxMethod)], !IO)
    ;
        io__write_string("(", !IO),
        output_type(Type, !IO),
        io__write_string(") ", !IO),
...
:- pred output_switch_cases(indent::in, module_info::in, func_info::in,
    mlds__context::in, list(mlds__switch_case)::in, mlds__switch_default::in,
...
:- pred output_switch_case(indent::in, module_info::in, func_info::in,
    mlds__context::in, mlds__switch_case::in, exit_methods::out,
    io::di, io::uo) is det.
...
output_switch_case(Indent, ModuleInfo, FuncInfo, Context, Case, ExitMethods,
    ...
    ModuleName = FuncInfo ^ func_info_name ^ mod_name,
    list__foldl(output_case_cond(Indent, ModuleInfo, ModuleName, Context),
        Conds, !IO),
    output_statement(Indent + 1, ModuleInfo, FuncInfo, Statement,
        StmtExitMethods, !IO),
    ( set__member(can_fall_through, StmtExitMethods) ->
...
:- pred output_case_cond(indent::in, module_info::in, mlds_module_name::in,
    mlds__context::in, mlds__case_match_cond::in, io::di, io::uo) is det.
...
:- pred output_switch_default(indent::in, module_info::in, func_info::in,
    mlds__context::in, mlds__switch_default::in, exit_methods::out,
    io::di, io::uo) is det.
