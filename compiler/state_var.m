...
%---------------------------------------------------------------------------%
    ...
    % When there is an illegal reference to as state variable
    % (as in e.g !A = f), then we ensure that later references
    % to e.g. !.A don't get avalance errors by pretending that
    % the state variable A is already known.
    %
:- pred make_svar_magically_known(svar::in, svar_state::in, svar_state::out,
...
make_svar_magically_known(StateVar, !SVarState, !UrInfo) :-
...
svar_finish_if_then_else(LocKind, Context, QuantStateVars,
    ...
    ;
        ...
        report_missing_inits_in_ite(Context, ThenMissingInits,
            "succeeds", "fails", MissingInThenSpec),
        store_missing_init_in_ite_report(MissingInThenSpec, !UrInfo)
    ),
    (
        ElseMissingInits = []
    ;
        ...
        report_missing_inits_in_ite(Context, ElseMissingInits,
            "fails", "succeeds", MissingInElseSpec),
        store_missing_init_in_ite_report(MissingInElseSpec, !UrInfo)
    ),
...
:- pred store_missing_init_in_ite_report(warn_spec::in,
    unravel_info::in, unravel_info::out) is det.

store_missing_init_in_ite_report(Spec, !UrInfo) :-
