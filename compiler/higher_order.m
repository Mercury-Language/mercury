...
:- import_module special_pred, passes_aux, inst_table.
...
interpret_typeclass_info_manipulator(Manipulator, Args,
	...
	->
		...
		Goal = unify(TypeInfoVar, var(TypeInfoArg),
			out_inst_pair - in_inst_pair,
...
specialize_special_pred(CalledPred, _CalledProc, Args,
	...
	->
		...
		InInUniMode = in_inst_pair - in_inst_pair,
		Goal = unify(Arg1, var(Arg2), InInUniMode,
...
unwrap_no_tag_arg(WrappedType, Constructor, Arg, UnwrappedArg,
	...
	UniModes = [(ground(shared, no) - free(unique)) ->
	...
	instmap_delta_from_assoc_list([UnwrappedArg - ground(shared, no)],
		InstMapDelta),
	goal_info_init(NonLocals, InstMapDelta, det, GoalInfo),
	Goal = unify(Arg, functor(ConsId, [UnwrappedArg]),
		in_inst_pair - out_inst_pair,
...
create_new_proc(ModuleInfo, NewPred, NewProcInfo0, NewPredInfo0, NewPredInfo,
	...
	%
	proc_info_get_initial_instmap(NewProcInfo7, ModuleInfo, InstMap0),
	find_class_context(InstMap0, InstTable0, ModuleInfo, ArgTypes,
...
construct_higher_order_terms(ModuleInfo, HeadVars0, HeadVars, ArgModes0,
	...
	->
		...
		proc_info_argmodes(CalledProcInfo,
			argument_modes(_CalledArgIT0, CalledArgModes)),
...
:- pred find_class_context(instmap::in, inst_table::in, module_info::in,
	list(type)::in, list(mode)::in, list(class_constraint)::in,
	list(class_constraint)::in, class_constraints::out) is det.

find_class_context(_, _, _, [], [], Univ0, Exist0, Constraints) :-
	list__reverse(Univ0, Univ),
	list__reverse(Exist0, Exist),
	Constraints = constraints(Univ, Exist).
find_class_context(_, _, _, [], [_|_], _, _, _) :-
	error("higher_order:find_class_context").
find_class_context(_, _, _, [_|_], [], _, _, _) :-
	error("higher_order:find_class_context").
find_class_context(InstMap, InstTable, ModuleInfo, [Type | Types],
		[Mode | Modes], Univ0, Exist0, Constraints) :-
	...
	find_class_context(InstMap, InstTable, ModuleInfo, Types, Modes,
