...
report_used_file_read_error(Globals, ModuleName, UsedFileError, Stream, !IO) :-
    ...
    ;
        ...
        list.map(extract_spec_msgs_and_maybe_add_id(Globals),
            coerce(one_or_more_to_list(OoMUsedItemSpecs)), MsgsList),
        list.condense(MsgsList, Msgs),
        Spec = gen_spec($pred, Severity, phase_read_files, Msgs)
    ),
    write_diag_spec(Stream, Globals, Spec, !IO).   % informational only
