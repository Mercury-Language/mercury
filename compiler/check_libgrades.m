...
:- pred detect_stdlib_grades(io.text_output_stream::in, globals::in,
    maybe1el(set(string))::out, io::di, io::uo) is det.
...
can_you_read_dir(MerStdLibDir, MaybeMerStdLibDir, !IO) :-
    ...
    ;
        ...
        MaybeMerStdLibDir = error1(one_or_more(Spec, []))
    ).
...
%---------------------------------------------------------------------------%
    ...
    %
:- pred check_chosen_stdlib_dir_exists(maybe(string)::in,
    maybe1el(string)::out, io::di, io::uo) is det.

check_chosen_stdlib_dir_exists(MaybeChosenStdLibDir, MaybeMerStdLibDir, !IO) :-
    (
        MaybeChosenStdLibDir = yes(ChosenStdLibDir),
        can_you_read_dir(ChosenStdLibDir, MaybeMerStdLibDirOoM, !IO),
        maybe1_to_maybe1el(MaybeMerStdLibDirOoM, MaybeMerStdLibDir)
    ;
