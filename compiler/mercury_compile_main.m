...
real_main_after_expansion(CmdLineArgs, !IO) :-
    ...
    ( if CmdLineArgs = ["--arg-file", ArgFile | ExtraArgs] then
        process_options_arg_file(ArgFile, ExtraArgs, DetectedGradeFlags,
            Variables, MaybeMCFlags, OptionArgs, NonOptionArgs,
            OptionSpecs, !IO)
    else
        process_options_plain(CmdLineArgs, DetectedGradeFlags,
            Variables, MaybeMCFlags, OptionArgs, NonOptionArgs,
            OptionSpecs, !IO)
    ),
    (
        ...
        write_error_specs(ActualGlobals, OptionSpecs, !IO),
    ...
    ;
        ...
        % in options_file.m.
        write_error_specs(DummyGlobals, OptionSpecs, !IO),
        io.set_exit_status(1, !IO)
    ).
...
:- pred process_options_plain(list(string)::in,
    list(string)::out, options_variables::out, maybe(list(string))::out,
    list(string)::out, list(string)::out, list(error_spec)::out,
    io::di, io::uo) is det.

process_options_plain(CmdLineArgs, DetectedGradeFlags, Variables,
        MaybeMCFlags, OptionArgs, NonOptionArgs, Specs, !IO) :-
    ...
    (
        OptFileErrors = no,
        process_options_plain_opt_file_ok(CmdLineArgs, ArgsGlobals, EnvVarMap,
            WarnUndef, DetectedGradeFlags, Variables0, Variables, MaybeMCFlags,
            OptFileOkSpecs, !IO),
...
:- pred process_options_plain_opt_file_ok(list(string)::in, globals::in,
    environment_var_map::in, bool::in, list(string)::out,
    options_variables::in, options_variables::out, maybe(list(string))::out,
...
do_process_compiler_cmd_line_args(Globals, OpModeArgs, OptionArgs,
        [Arg | Args], !Modules, !ExtraObjFiles, !HaveReadModuleMaps, !IO) :-
    % `mmc --make' has already set up the options.
...
process_augmented_module(Globals0, OpModeAugment, Baggage, AugCompUnit,
    ...
    make_hlds_pass(Globals, OpModeAugment, WriteDFile,
