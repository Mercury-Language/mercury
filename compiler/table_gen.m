...
create_new_loop_goal(OrigGoal, Statistics, PredId, ProcId,
    ...
    table_generate_foreign_proc(ModuleInfo, MarkInactivePredName,
    ...
    table_generate_foreign_proc(ModuleInfo, MarkInactiveFailPredName,
    ...
    table_generate_foreign_proc(ModuleInfo, MarkActiveFailPredName,
        [TableTipArg], [], instmap_delta_bind_no_var, detism_failure,
...
create_new_memo_goal(Detism, OrigGoal, Statistics, _MaybeSizeLimit,
    ...
    ;
        ...
        MarkAsFailedCode = MarkAsFailedMacroName ++
            "(" ++ DebugArgStr ++ ", " ++ cur_table_node_name ++ ");",
        table_generate_foreign_proc(ModuleInfo, MarkAsFailedPredName,
            [TableTipArg], [], instmap_delta_bind_no_var, detism_failure,
...
create_new_memo_non_goal(Detism, OrigGoal, Statistics, _MaybeSizeLimit,
    ...
    table_generate_foreign_proc(ModuleInfo, MarkIncompletePredName,
    ...
    table_generate_foreign_proc(ModuleInfo, MarkActivePredName,
    ...
    table_generate_foreign_proc(ModuleInfo, MarkCompletePredName,
        [RecordArg], [], instmap_delta_bind_no_var, detism_failure,
...
create_new_io_goal(OrigGoal, TableIoEntryKind, Unitize, TableIoStates,
    ...
    generate_new_table_var("StartVar", int_type, !VarSet, !VarTypes,
        StartVar),
    table_generate_call(ModuleInfo, "table_io_in_range",
    ...
    generate_new_table_var("TipVar", trie_node_type, !VarSet, !VarTypes,
        TipVar),
    table_generate_call(ModuleInfo, "table_lookup_insert_start_int",
        [TableVar, StartVar, CounterVar, TipVar],
        instmap_delta_bind_var(TipVar),
        detism_det, purity_impure, Context, LookupGoal),
    table_generate_call(ModuleInfo, "table_io_has_occurred",
    ...
    ;
        ...
        table_generate_call(ModuleInfo, "table_io_copy_io_state",
    ...
    ;
        ...
        table_generate_call(ModuleInfo, "table_io_left_bracket_unitized_goal",
            [SavedTraceEnabledVar],
            instmap_delta_bind_var(SavedTraceEnabledVar),
            detism_det, purity_impure, Context, LeftBracketGoal),
        table_generate_call(ModuleInfo, "table_io_right_bracket_unitized_goal",
...
create_new_mm_goal(Detism, OrigGoal, Statistics, PredId, ProcId,
    ...
    table_generate_call(ModuleInfo, "table_mm_completion",
...
do_own_stack_transform(Detism, OrigGoal, Statistics, PredId, ProcId,
    ...
    SetupCode = "\t" ++ generator_name ++ " = " ++
        ...
        "\t" ++ consumer_name ++ " = " ++
            ...
            ", """ ++ PredName ++ """);\n",
    table_generate_foreign_proc(ModuleInfo, SetupPredName,
    ...
    table_generate_call(ModuleInfo, ConsumePredName,
    ...
    table_generate_foreign_proc(ModuleInfo, RestoreAllPredName,
...
do_own_stack_create_generator(PredId, ProcId, !.PredInfo, !.ProcInfo,
    ...
    table_generate_foreign_proc(ModuleInfo0, "table_mmos_pickup_inputs",
    ...
    CompletionArg = foreign_arg(GeneratorVar,
        ...
        generator_type, bp_native_if_possible),
    table_generate_foreign_proc(ModuleInfo0, "table_mmos_completion",
...
generate_simple_call_table_lookup_goal(StatusType, PredName,
    ...
    CodeStr =
        ...
        MainPredCodeStr,
    table_generate_foreign_proc(ModuleInfo, PredName,
...
generate_memo_save_goal(NumberedSaveVars, TableTipVar, BlockSize,
    ...
    ( if BlockSize > 0 then
        ...
        table_generate_foreign_proc(ModuleInfo, CreatePredName,
        ...
        MarkAsSucceededCode = MarkAsSucceededMacroName ++
            "(" ++ DebugArgStr ++ ", " ++ cur_table_node_name ++ ");",
        table_generate_foreign_proc(ModuleInfo, MarkAsSucceededPredName,
...
generate_memo_non_save_goals(NumberedSaveVars, PredId, ProcId,
    ...
    CodeStr = LookupDeclCodeStr ++ SaveDeclCodeStr ++ "\n" ++
        ...
        "\tif (" ++ SuccName ++ ") {\n" ++ CreateSaveCode ++ "\t}\n" ++
        AssignSuccessCodeStr,
    table_generate_foreign_proc(ModuleInfo, DuplCheckPredName,
...
generate_memo_restore_goal(NumberedOutputVars, OrigInstMapDelta, TipVar,
    ...
    (
        ...
        GetRestoreCodeStr = "\t" ++ GetMacroName ++ "(" ++
            ...
            answer_block_name ++ ");\n" ++
            RestoreCodeStr,
        table_generate_foreign_proc(ModuleInfo, GetPredName,
            Args, RestoreArgs,
            instmap_delta_from_assoc_list(RestoreInstMapDeltaSrc),
...
generate_memo_non_restore_goal(Detism, NumberedOutputVars, OrigInstMapDelta,
    ...
    generate_new_table_var("AnswerBlock", answer_block_type,
        !VarSet, !VarTypes, AnswerBlockVar),
    ModuleInfo = TableInfo ^ table_module_info,
    table_generate_call(ModuleInfo, ReturnAllAns,
...
generate_mm_restore_or_suspend_goal(PredName, Detism, Purity,
    ...
    generate_new_table_var("AnswerBlock", answer_block_type,
        !VarSet, !VarTypes, AnswerBlockVar),
    ModuleInfo = TableInfo ^ table_module_info,
    table_generate_call(ModuleInfo, PredName,
    ...
    table_generate_foreign_proc(ModuleInfo, ReturnAllPredName,
        Args, RestoreArgs,
        instmap_delta_from_assoc_list(RestoreInstMapDeltaSrc),
        detism_det, purity_semipure, tabling_c_attributes_no_dupl,
...
generate_error_goal(TableInfo, Context, Msg, !VarSet, !VarTypes, Goal) :-
    ...
    table_generate_call(ModuleInfo, "table_error",
...
:- pred table_generate_call(module_info::in, string::in, list(prog_var)::in,
    instmap_delta::in, determinism::in, purity::in, term.context::in,
    hlds_goal::out) is det.

table_generate_call(ModuleInfo, PredName, ArgVars, InstMapDelta,
    ...
    generate_simple_call(ModuleInfo, BuiltinModule, PredName, pf_predicate,
        only_mode, Detism, Purity, [], ArgVars, Features, InstMapDelta,
        Context, Goal).

:- pred table_generate_foreign_proc(module_info::in, string::in,
...
table_generate_foreign_proc(ModuleInfo, PredName, Args, ExtraArgs,
        InstMapDelta, Detism, Purity, Attributes, Code, Context, Goal) :-
    (
        Purity = purity_pure,
        Features0 = []
    ;
    ...
    goal_util.generate_foreign_proc( ModuleInfo,BuiltinModule, PredName,
        pf_predicate, only_mode, Detism, Purity, Attributes,
        [], Args, ExtraArgs, MaybeTraceRuntimCond, Code, Features,
        InstMapDelta, Context, Goal).
