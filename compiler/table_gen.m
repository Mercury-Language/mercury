...
:- pred table_gen__create_new_det_goal(eval_method::in, hlds_goal::in,
	pred_id::in, proc_id::in, instmap::in, list(prog_var)::in,
	argument_modes::in, map(prog_var, type)::in, map(prog_var, type)::out,
...
table_gen__create_new_det_goal(EvalMethod, OrigGoal, PredId, ProcId,
		InstMap0, HeadVars, HeadVarModes, VarTypes0, VarTypes,
...
generate_loop_error_goal(TableInfo, Context, VarTypes0, VarTypes,
	...
	set__init(NonLocals),
	create_instmap_delta([MessageConsGoal, CallGoal], InstMapDelta),
...
:- pred get_input_output_vars(list(prog_var)::in, list(mode)::in, instmap::in,
		inst_table::in, module_info::in,
...
get_input_output_vars([], [_|_], _, _, _, _, _) :-
	error("get_input_output_vars: lists not same length").
get_input_output_vars([Var | RestV], [Mode | RestM], InstMap, InstTable,
		Module, InVars, OutVars) :-
	(
