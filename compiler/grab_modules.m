...
:- pred grab_qual_imported_modules_augment(io.text_output_stream::in,
    ...
    %   BlockErrSpecs, !Baggage, !AugCompUnit, !HaveParseTreeMaps, !IO):
    %
...
:- pred grab_trans_opt_files(io.text_output_stream::in, globals::in,
    list(module_name)::in, list(err_spec)::out,
    module_baggage::in, module_baggage::out,
    aug_compilation_unit::in, aug_compilation_unit::out,
    have_parse_tree_maps::in, have_parse_tree_maps::out,
    io::di, io::uo) is det.
...
grab_unqual_imported_modules_make_int(ProgressStream, Globals,
    ...
    % The predicates grab_qual_imported_modules_augment and
    ...
    (
        ...
        grab_module_int0_files_for_amiu(ProgressStream, Globals,
            ...
            set.init, AncestorIntImports0, set.init, AncestorImpImports0,
        ...
        grab_module_int3_files(ProgressStream, Globals,
            "unqual_parent_int_imported", rwi3_direct_ancestor_int_import,
            set.to_sorted_list(AncestorIntImports),
            !IntIndirectImported, !HaveParseTreeMaps,
            !Baggage, !AugMakeIntUnit, !IO),
        grab_module_int3_files(ProgressStream, Globals,
            "unqual_parent_imp_imported", rwi3_direct_ancestor_imp_import,
            set.to_sorted_list(AncestorImpImports),
        ...
        grab_module_int3_files(ProgressStream, Globals,
            "unqual_parent_int_used", rwi3_direct_ancestor_int_use,
            set.to_sorted_list(AncestorIntUses),
            !IntIndirectImported, !HaveParseTreeMaps,
            !Baggage, !AugMakeIntUnit, !IO),
        grab_module_int3_files(ProgressStream, Globals,
            "unqual_parent_imp_used", rwi3_direct_ancestor_imp_use,
            set.to_sorted_list(AncestorImpUses),
            !ImpIndirectImported, !HaveParseTreeMaps,
            !Baggage, !AugMakeIntUnit, !IO),
        grab_module_int3_files(ProgressStream, Globals,
        ...
        !AugMakeIntUnit ^ amiu_delayed_specs := coerce(DelayedSpecs),
...
grab_qual_imported_modules_augment(ProgressStream, Globals, MaybeTimestamp,
    ...
    (
        ...
        grab_module_int0_files_for_acu(ProgressStream, Globals,
            "ancestors", rwi0_section, Ancestors,
            set.init, AncestorIntImports0, set.init, AncestorIntUses0,
        ...
        grab_module_int1_files(ProgressStream, Globals,
            "ancestor_int_imported", rwi1_ancestor_int_import,
            set.to_sorted_list(AncestorIntImports),
            !IntIndirectImported, !IntImpIndirectImported,
            !HaveParseTreeMaps, !Baggage, !AugCompUnit, !IO),
        grab_module_int1_files(ProgressStream, Globals,
            "ancestor_imp_imported", rwi1_ancestor_imp_import,
            set.to_sorted_list(AncestorImpImports),
        ...
        grab_module_int1_files(ProgressStream, Globals,
            "ancestor_int_used", rwi1_ancestor_int_use,
            set.to_sorted_list(AncestorIntUses),
            !IntIndirectImported, !IntImpIndirectImported,
            !HaveParseTreeMaps, !Baggage, !AugCompUnit, !IO),
        grab_module_int1_files(ProgressStream, Globals,
            "ancestor_imp_used", rwi1_ancestor_imp_use,
            set.to_sorted_list(AncestorImpUses),
            !ImpIndirectImported, !ImpImpIndirectImported,
            !HaveParseTreeMaps, !Baggage, !AugCompUnit, !IO),
        grab_module_int1_files(ProgressStream, Globals,
...
grab_plain_opt_and_int_for_opt_files(ProgressStream, Globals,
    ...
    ( if (UnusedArgs = opt_unused_args_intermod ; StructureReuse = yes) then
        ...
        (
            ...
            report_any_errors_in_read_opt_file(OwnOptModuleErrors,
                !BlockErrSpecs, WarnSpecs0, WarnSpecs)
        ;
            ...
            report_cannot_read_opt_file(Globals, warn_missing_opt_files,
                ...
                WarnSpecs0, WarnSpecs)
        )
    else
        ParseTreePlainOpts = ParseTreePlainOpts0,
        WarnSpecs = WarnSpecs0
    ),
    ...
    grab_module_int0_files_for_acu(ProgressStream, Globals,
        ...
        set.init, OptAncestorIntImports, set.init, OptAncestorIntUses,
        set.init, OptAncestorImpImports, set.init, OptAncestorImpUses,
        !HaveParseTreeMaps, !Baggage, !AugCompUnit, !IO),
    set.union(OptAncestorIntImports, OptAncestorImpImports,
        OptAncestorImports),
    set.union(OptAncestorIntUses, OptAncestorImpUses,
        OptAncestorUses),
    ...
    %
    % Instead of returning a list of warn_specs, we used to return a flag
    ...
    !:BlockErrSpecs =
        ModuleErrors ^ rm_fatal_err_specs ++
        % ModuleErrors ^ rm_nonfatal_err_specs ++     see above
        !.BlockErrSpecs,

    % This adds NonBlockingSpecs to ModuleErrors ^ rm_nonfatal_err_specs.
    % If we ever want to go back to the old behavior of considering
    % ModuleErrors ^ rm_nonfatal_err_specs being nonempty as grounds for
    % stopping compiler execution before semantic analysis, then we would
    % want the computation of !:BlockErrSpecs to use the value of that field
    ...
    % !.BlockErrSpecs (i.e. the version from before adding
    % ModuleErrors ^ rm_fatal_warn_specs) to the baggage as well.
    module_baggage_add_warnings(WarnSpecs, !Baggage).

%---------------------------------------------------------------------------%

grab_trans_opt_files(ProgressStream, Globals, TransOptModuleNames,
        BlockErrSpecs, !Baggage, !AugCompUnit,
...
grab_module_int0_files_for_acu(_ProgressStream, _Globals, _Why, _ReadWhy0,
        [], !DirectIntImports, !DirectIntUses,
        !DirectImpImports, !DirectImpUses,
        !HaveParseTreeMaps, !Baggage, !AugCompUnit, !IO).
grab_module_int0_files_for_acu(ProgressStream, Globals, Why, ReadWhy0,
        [ModuleName | ModuleNames], !DirectIntImports, !DirectIntUses,
        !DirectImpImports, !DirectImpUses,
        !HaveParseTreeMaps, !Baggage, !AugCompUnit, !IO) :-
    ( if should_read_interface(!.Baggage, ModuleName, ifk_int0) then
        ...
        grab_module_int0_file_for_acu(ProgressStream, Globals, ReadWhy0,
            ...
            !HaveParseTreeMaps, !Baggage, !AugCompUnit, !IO),
        set.union(IntImports, !DirectIntImports),
        set.union(ImpImports, !DirectImpImports),
        set.union(IntUses, !DirectIntUses),
        set.union(ImpUses, !DirectImpUses)
    else
        maybe_log_augment_decision(ProgressStream, Why, ifk_int0, ReadWhy0,
...
grab_module_int0_files_for_amiu(_ProgressStream, _Globals, _Why, [],
        !DirectIntImports, !DirectImpImports, !DirectIntUses, !DirectImpUses,
        !HaveParseTreeMaps, !Baggage, !AugMakeIntUnit, !IO).
...
:- pred module_baggage_add_nonfatal_specs(list(err_spec)::in,
    module_baggage::in, module_baggage::out) is det.

module_baggage_add_nonfatal_specs(NewSpecs, !Baggage) :-
    Errors0 = !.Baggage ^ mb_errors,
    NonFatalErrorSpecs0 = Errors0 ^ rm_nonfatal_err_specs,
    NonFatalErrorSpecs = NewSpecs ++ NonFatalErrorSpecs0,
    Errors = Errors0 ^ rm_nonfatal_err_specs := NonFatalErrorSpecs,
    !Baggage ^ mb_errors := Errors.

:- pred module_baggage_add_warnings(list(warn_spec)::in,
    module_baggage::in, module_baggage::out) is det.

module_baggage_add_warnings(NewSpecs, !Baggage) :-
    Errors0 = !.Baggage ^ mb_errors,
    WarnSpecs0 = Errors0 ^ rm_warn_specs,
    WarnSpecs = NewSpecs ++ WarnSpecs0,
    Errors = Errors0 ^ rm_warn_specs := WarnSpecs,
    !Baggage ^ mb_errors := Errors.
...
read_plain_opt_files(_, _, _, _, [], _, !ParseTreePlainOptsCord,
        !ExplicitDeps, !ImplicitNeeds, !BlockErrSpecs, !WarnSpecs, !IO).
read_plain_opt_files(ProgressStream, Globals, VeryVerbose,
    ...
    (
        ...
        report_any_errors_in_read_opt_file(ModuleErrors,
            !BlockErrSpecs, !WarnSpecs),
    ...
    ;
        ...
        report_cannot_read_opt_file(Globals, warn_missing_opt_files,
            FileName, ModuleFileId, ModuleErrors, !WarnSpecs),
        ModuleNames1 = ModuleNames0,
        DontQueueOptModules1 = DontQueueOptModules0
    ),
...
read_trans_opt_files(ProgressStream, Globals, [ModuleName | ModuleNames],
        !ParseTreeTransOptsCord, !BlockErrSpecs, !WarnSpecs, !IO) :-
...
report_cannot_read_opt_file(Globals, WarnOption, FileName, ModuleFileId,
        ReadModuleErrors, !WarnSpecs) :-
    % We get here if we couldn't find and/or open the file.
    % ModuleErrors ^ rm_fatal_warn_specs will already contain
    % an error_severity warn_spec about that, with more details
    ...
    % any warn_specs from ModuleErrors to !Specs here.
    %
    ...
    ;
        ...
        Spec = no_ctxt_spec($pred, Severity, Phase, Pieces),
        !:WarnSpecs = [Spec | !.WarnSpecs]
    ).
    % NOTE: We do NOT update !BlockErrSpecs, since a missing optimization
    % interface file is not necessarily an error.

    % report_any_errors_in_read_opt_file(ModuleErrors, !BlockErrSpecs):
    %
    % Work out whether any errors have occurred while reading
    % a .opt or .trans_opt file, and update !BlockErrSpecs accordingly.
    % Note that
    %
...
report_any_errors_in_read_opt_file(ModuleErrors, !BlockErrSpecs, !WarnSpecs) :-
    ...
    then
        ...
        get_read_module_specs(ModuleErrors, ModuleErrSpecs, ModuleWarnSpecs),
        expect_not(unify(ModuleErrSpecs, []), $pred, "ModuleErrSpecs = []"),
        !:BlockErrSpecs = ModuleErrSpecs ++ !.BlockErrSpecs,
        !:WarnSpecs = ModuleWarnSpecs ++ !.WarnSpecs
    ).
