...
:- func ucount(sparse_bitset(T)) = uint <= uenum(T).
...
ucount(Set) = sparse_bitset.foldl((func(_, Acc) = Acc + 1u), Set, 0u).
...
:- mode do_foldl_pred(
    in(pred(in, mdi, muo) is det), in, mdi, muo) is det.
:- mode do_foldl_pred(
    in(pred(in, di, uo) is det), in, di, uo) is det.
:- mode do_foldl_pred(
    in(pred(in, in, out) is semidet), in, in, out) is semidet.
:- mode do_foldl_pred(
    in(pred(in, mdi, muo) is semidet), in, mdi, muo) is semidet.
:- mode do_foldl_pred(
    in(pred(in, di, uo) is semidet), in, di, uo) is semidet.
:- mode do_foldl_pred(
    in(pred(in, in, out) is nondet), in, in, out) is nondet.
:- mode do_foldl_pred(
    in(pred(in, in, out) is cc_multi), in, in, out) is cc_multi.
:- mode do_foldl_pred(
