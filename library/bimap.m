...
:- pred bimap__map_values(pred(K, V, W)::in(pred(in, in, out) is det),
    ...
    %
:- func bimap__foldl(func(K, V, A) = A, bimap(K, V), A) = A.
:- pred bimap__foldl(pred(K, V, A, A), bimap(K, V), A, A).
...
:- mode bimap__foldl(pred(in, in, di, uo) is det, in, di, uo) is det.
    ...
    %
:- pred bimap__foldl2(pred(K, V, A, A, B, B), bimap(K, V), A, A, B, B).
...
:- mode bimap__foldl2(pred(in, in, di, uo, di, uo) is det,
    ...
    % and a little more efficient).
    %
:- pred bimap__foldl3(pred(K, V, A, A, B, B, C, C), bimap(K, V),
    A, A, B, B, C, C).
